...
from writracker.recorder.io import _parse_config_int_value, _parse_config_float_value, _parse_config_bool_value
...
trials_index_fields = 'trial_num', 'target_num', 'sub_trial_num', 'target', 'response', 'start_time', 'rc'
...
def load_experiment(dir_name):
    ...
    trials = []
    for t in index:
        trial = wt.data.CodedTrial(trial_num=t['trial_num'], target_num=t['target_num'], stimulus=t['target'],
        ...
        characters, strokes = _load_trajectory(trial.trial_num, dir_name + os.sep + traj_filenames[trial_key])
...
def save_experiment(exp, dir_name):
    ...
    wt.recorder.io.reset_trial_info_file(dir_name)

    for trial in exp.trials:
        append_to_trial_index(dir_name, trial.trial_num, trial.sub_trial_num, trial.target_num, trial.stimulus,
                              trial.response, trial.start_time, trial.rc)
        _set_char_num_in_each_stroke(trial)
        save_trajectory(trial.strokes, trial.trial_num, trial.sub_trial_num, dir_name)
...
def _load_trajectory_filenames(dir_name):
    ...
    for fn in os.listdir(dir_name):
        ...
        filenames[(trial_num, sub_trial_num)] = fn

    return filenames


#-------------------------------------------------------------------------------------
def _load_trajectory(trial_num, filename):

    if not os.path.isfile(filename):
        raise Exception('Error loading trial #{}: File {} does not exist'.format(trial_num, filename))
    ...
    with open(filename, 'r') as fp:
        ...
        for row in reader:
            ...
            if stroke_num != last_stroke_num:
                ...
                curr_stroke = wt.data.Stroke(on_paper, char_num, [])
...
def _strokes_to_characters(filename, strokes_info):
    ...
    for stroke, char_num in strokes_info:
        ...
        if char_num == 0:
            ...
            # -- New character
            curr_char = wt.data.Character(char_num, [stroke], pre_char_space=pre_char_space)
            pre_char_space = None
...
#-------------------------------------------------------------------------------------------------
def append_to_trial_index(dir_name, trial_num, sub_trial_num, target_num, target, response, trial_start_time, rc):
    """
    Append a line to the trials.csv file
    """

    remove_from_trial_index(dir_name, trial_num, sub_trial_num)

    index_fn = trial_index_filename(dir_name)
    file_exists = os.path.isfile(index_fn)

    entry = dict(trial_num=trial_num,
    ...
    with open(index_fn, 'a' if file_exists else 'w') as fp:
        writer = csv.DictWriter(fp, trials_index_fields, lineterminator=u.newline())
        if not file_exists:
            writer.writeheader()
...
#----------------------------------------------------------
def remove_from_trial_index(dir_name, trial_num, sub_trial_num=None):
    """
    ...
    with open(index_fn, 'w') as fp:
        ...
        writer.writeheader()
        for entry in index:
            to_remove = trial_num == entry['trial_num'] and (sub_trial_num is None or entry['sub_trial_num'] == sub_trial_num)
            if not to_remove:
                writer.writerow(entry)
...
def load_trials_index(dir_name):
    ...
    with open(index_fn, 'r') as fp:
        ...
        for row in reader:
            ...
            trial_num = _parse_config_int_value('trial_num', row['trial_num'], location)
            sub_trial_num = _parse_config_int_value('sub_trial_num', row['sub_trial_num'], location)
            target_num = _parse_config_int_value('target_num', row['target_num'], location, allow_empty=True)
            start_time = _parse_config_float_value('start_time', row['start_time'], location, allow_empty=True)
            rc = None if row['rc'] == '' else row['rc']
            target = row['target']
            response = row['response']

            result.append(dict(trial_num=trial_num, sub_trial_num=sub_trial_num,
                               target_num=target_num, target=target, response=response,
                               start_time=start_time, rc=rc))
